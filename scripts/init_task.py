...
from utils.utils import  delete_folder, copy_folder, dict_to_text, extract_all_kernels_flat, extract_cuda_kernel_names, find_best_match, list_all_files, load_show_files, read_file, remove_justification, text_to_dict, write_file, extract_recommendation
...
#
        #    navie_branch = task_root / "optimize" / "branch_0"
#
        #    file_name_list = list_all_files(baseline_folder)
        #    file_name_list.remove("kernel.cu")
        #    file_name_list.remove("ncu_report.csv")
#
        #    for file_name in file_name_list:
        #        kernel_name = extract_cuda_kernel_names(Path(baseline_folder / file_name))[0]
        #        dir_name = file_name.removesuffix(".cu")
#
        #        iter = 0
        #        while iter < args.optimize_iter:
        #            currnt_dir = navie_branch / dir_name / f"iter_{iter}"
        #            if iter == 0:
        #                shutil.copy2(baseline_folder / file_name, currnt_dir / file_name)
        #                profile_with_ncu(task_root / "spec" / "entry.py", args.device, current_dir / "ncu_report.csv", kernel_names=[kernel_name])
        #            else:
        #                pass
